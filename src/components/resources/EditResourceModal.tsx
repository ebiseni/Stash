...
import { updateResource, closeEditModal } from '../../features/resources/resourceSlice';
...
function EditResourceModal() {
  ...
  const editingResource = useAppSelector(state => state.resources.editingResource);

  const [title, setTitle]               = useState(editingResource?.title ?? '');
  const [url, setUrl]                   = useState(editingResource?.url ?? '');
  const [collectionId, setCollectionId] = useState(editingResource?.collectionId ?? '');
  const [tagInput, setTagInput]         = useState('');
  const [tags, setTags]                 = useState<string[]>(editingResource?.tags ?? []);
  const [quickNote, setQuickNote]       = useState(editingResource?.quickNote ?? '');
  ...
  const validate = () => {
    ...
    if (!collectionId)     newErrors.collection = 'Please select a collection.';
  ...
  const detectType = (urlStr: string): ResourceType => {
    ...
    if (lower.includes('youtube') || lower.includes('vimeo')) return ResourceType.Video;
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-card modal-card--wide">
        ...
        <h2 className="modal-card__heading">Edit Resource</h2>
        ...
        <div className="modal-card__field">
          ...
          />
          {errors.title && <span className="modal-card__error">{errors.title}</span>}
        </div>
        ...
        <div className="modal-card__field">
          ...
          />
          {errors.url && <span className="modal-card__error">{errors.url}</span>}
        </div>
        ...
        <div className="modal-card__field">
          ...
          <select
            ...
            onChange={e => {
              ...
              if (errors.collection) setErrors(p => ({ ...p, collection: undefined }));
            }}
          >
        ...
        <div className="modal-card__field">
          ...
          <div className={`modal-card__tag-input ${errors.tags ? 'modal-card__input--error' : ''}`}>
            {tags.map(tag => (
          ...
          <span className="modal-card__hint">Press comma or Enter to add a tag</span>
          {errors.tags && <span className="modal-card__error">{errors.tags}</span>}
        </div>
